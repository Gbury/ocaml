...
let box_float dbg c = Cop(Calloc [| Int; Float |], [alloc_float_header dbg; c], dbg)
...
let rec unbox_float dbg cmm =
  match cmm with
  | Cop(Calloc [| Int; Float |], [Cblockheader (header, _); c], _) when header = float_header ->
      c
...
let box_complex dbg c_re c_im =
  Cop(Calloc [| Int; Float; Float |], [alloc_floatarray_header 2 dbg; c_re; c_im], dbg)
