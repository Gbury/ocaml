...
let simplify_let ~simplify_expr ~simplify_toplevel dacc let_expr ~down_to_up =
  ...
  L.pattern_match let_expr ~f:(fun bindable_let_bound ~body ->
    ...
    let dacc = DA.map_data_flow dacc ~f:(fun data_flow ->
      let data_flow = Data_flow.add_used_in_current_handler
                       free_names_in_lifted_constants data_flow
      in
      List.fold_left
        (fun acc (binding_to_place : Simplify_named_result.binding_to_place) ->
           let bindable_let_bound = binding_to_place.let_bound in
           let simplified_named = binding_to_place.simplified_defining_expr in
           ...
           | Reachable { free_names; named; cost_metrics = _; } ->
             ...
             if not can_be_removed then
               ...
               | Singleton v ->
                 Data_flow.record_binding
                   (Var_in_binding_pos.var v) free_names acc
