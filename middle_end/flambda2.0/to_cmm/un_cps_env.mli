...
val mk :
  Un_cps_closure.env -> Continuation.t -> Continuation.t -> t
(** [mk offsets k k_exn] creates a local environment for translating
    a flambda2 expression, with return continuation [k],
    and exception continuation [k_exn]. *)
...
val add_jump_cont :
  t -> (Variable.t * Cmm.machtype) list -> Continuation.t -> int * t
